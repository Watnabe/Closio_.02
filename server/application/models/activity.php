...
class Activity extends Model
{
    ...
    function save(){
        ...
        if(isset($this->linked_object_type))
        {
            ...
            if($this->linked_object_type == 'deal'){
                $this->set('deal_id', $this->linked_object_id);
            }
        }
    ...
    function set_object($object)
    {
        ...
        if(isset($object->deal_id))
            $this->set('deal_id', $object->deal_id);

        //if the object is a deal, the deal_id is the object id
        if($object instanceof Deal)
            $this->set('deal_id', $object->id);
    ...
    function get($criteria = null){
        ...
        $sql = "SELECT activity.*,
                ...
                LEFT JOIN deals on activity.deal_id = deals.id";

        $sql = isset($criteria) ? "$sql $criteria" : $sql;

        //todo: does this make sense for activity? Both clients and admins can access all activity for a deal. Plus it looks like it would build invalid sql with two where statements (see dealdetails class)

...
class AppEmail extends Email
{
    ...
    function send_proposal($client_id, $proposal)
    {
        ...
        foreach ($users as $user) {
            if ($user['role_id'] != 1) {
                ...
                $this->set_subject('email_subjects.new_proposal');
                $this->generate('new-proposal', get_object_vars($proposal));
                $this->send();
            }
        }
    ...
    function send_file_upload_notification($deal, $files)
    {
        $users = $this->get_all_other_users($deal->client_id);

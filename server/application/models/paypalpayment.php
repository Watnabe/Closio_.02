...
{
    //access is checked by the proposal
    ...
    public function generate_submission_button($proposal)
    {
        ...
        $item_name = get_config('company.name') . ' - proposal #' . $proposal->number;
        $item_number = $proposal->number;
        ...
        $return_url = $base_url . "#deals/$proposal->deal_id/proposals/$proposal->id";
        $cancel_url = $base_url . "#deals/$proposal->deal_id/proposals/$proposal->id";
    ...
    function record_successful_payment()
    {
        ...
        $sql = 'SELECT * FROM proposals WHERE number = ' . $data['item_number'];

        $db = Database::getInstance();
        $record_set = $db->select($sql);
        $proposal = $record_set->export();
        //$proposal = $this->select($sql);

        $proposal = isset($proposal[0]) ? $proposal[0] : false;

        if($proposal){
            $proposal = new proposal($proposal->id);

            $payment->set('amount', $data['payment_gross']);
            $payment->set('payment_processor_charge_id', $data['txn_id']);
            $payment->set('proposal_id', $proposal->id);
            $payment->set('user_id', $data['custom']);
        }


        try{
            $payment->set_primary_details($proposal, 'paypal');
            $payment->complete_payment($proposal, 'paypal', $data['custom']);
            $payment->send_payment_notification($proposal, 'paypal', $data['custom']);
        }
        catch(Exception $e){

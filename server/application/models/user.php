...
Class User extends Model
{
    ...
    function save(){
		...
		if($this->is_new()){
            ...
            if($current_user->is('client') && $current_user->id != $this->id)
                return false;
    ...
    function create(){
        ...
        if($auth->username_available($this->email)){
            ...
            $registration = $auth->register($this->id, $this->role, $current_user->id);
    ...
    function current_user_can_access(){
        ...
        if($user->role == 'admin' || $user->role == 'user' || $user->role == 'superadmin' || $user->client_id == $this->client_id)
            return true;
        else return false;
    }

...
Class Deal extends Model
{
    ...
    function save()
    {
        ...
        if ($this->is_new()) {
			$user = current_user();
            //new deals won't have any tasks so the progress should be 100
            ...
            if($this->due_date_changed()){
                ActivityManager::deal_due_date_updated($this);
            }
        }
    ...
    function evaluate_date_difference($from, $to){
        ...
        $difference_direction = $to > $from ? Language::get('deal.increased') : Language::get('deal.decreased');
        $days_text =  $difference_in_days > 1 ? Language::get('deal.days') : Language::get('deal.day');
    ...
    function files_folder_name()
    {
        ...
        $records = $this->select("SELECT id FROM deals WHERE file_folder = '$name'");
    ...
    function get($criteria = null)
    {
        ...
        if (is_numeric($criteria)) {
            ...
            $deal['status_text'] = $this->status_text;
            ...
            $deals =  parent::get($sql);

            foreach($deals as &$deal){
                //create the proposal
                $deal_object = new Deal($deal);
//todo: i don't think this makes any sense because the proposal status will already be updated when I create the proposal using new proposal
                //store the current deal status as it exists in the database
                $old_status = $deal_object->status_text;
                $old_expected_progress = $deal_object->expected_progress;

                $deal_object->clear_params();

                //update the deal status
                $deal_object->update_status();

                //if the old status and the new status do not match, then we need to save this task back to the database.
                if ($old_status != (string)$deal_object->status_text || $old_expected_progress != (int)$deal_object->expected_progress) {
                    //todo:make sure this isn't saving each time
                    //todo: I only want to update the expected progress if we're retreiving a single deal. Not when we're getting the list
                    $deal_object->save(false);
                }

                //we need to add the status text back the task array, since we're sending the array to the client,
                //not the object
                $deal['status_text'] = $deal_object->status_text;
            }
            return $deals;
    ...
    function get_tasks($filter = null)
    {
        ...
        $tasks = $task->get(" WHERE deal_id = $this->id $additional_criteria" , array('order', 'ASC'));
        return $tasks;
    }
    ...
    function get_proposals()
    {
        ...
        $proposals = $proposal->get("WHERE deal_id = $this->id");
    ...
    function get_files()
    {
        $sql = File::generate_base_sql_for_get() . " WHERE deal_id = $this->id";
    ...
    function get_activity(){
        ...
        return $activity->get('WHERE deal_id = ' . $this->id);
    }
    ...
    static function context($object_name, $id)
    {
        ...
        if ($object_name == 'deal' || $object_name == 'template') {
            $context['deal_id'] = $id;
            ...
            if(isset($object->deal_id)){
                $context['deal_id'] = $object->deal_id;
            }
    ...
    function get_available_task_weight(){
        $sql = "SELECT SUM(weight)
                FROM tasks
                WHERE deal_id = $this->id AND is_section = 0";
    ...
    function calculate_expected_progress()
    {
        ...
        if ($total_time == 0) {
            //if the deal begins and ends on the same day, we neeed to determine if that day is in the future or the past
    ...
    {
        //there is no way to assess status on deals without a due date
        if (!$this->has_due_date()) {
        ...
        //of tasks. This will be used to determine if the deal has been started or not (a task count greater than
        //0 indicates that the deal has started
        ...
        //if the due date has passed and the current progress isn't 100%, then the deal is overdue regardless
        ...
        if($this->due_date < time() && $this->progress < 100){
            ...
            if ($this->progress >= 100)
            {
                //if the task counts hasn't been passed in, we will need them to differentiate between a complete deal
                //and a deal that hasn't been started
                if(!isset($task_counts))
                    $task_counts = $this->get_task_counts();

                //if we're passing in the task counts and there arent' yet any tasks, the deal status should be not
                //started.
...
//
        ...
        if($old_status != $this->status_text){
            ActivityManager::deal_status_changed($this);
    ...
    function delete_deal_files(){
        ...
        $sql = "DELETE from files where deal_id = " . $this->id;
        $this->execute($sql);
    }

    function delete(){
        $sql_delete_tasks = "DELETE FROM tasks WHERE deal_id = " . $this->id;
        $this->execute($sql_delete_tasks);

        $this->delete_deal_files();

        $this->delete_file_folder();

        $result = parent::delete();

        ActivityManager::deal_deleted($this);

        return $result;
    }

    //used when getting a single deal
    function current_user_can_access(){
        $user = current_user();

        if(!$user->role == 'client' || $user->client_id == $this->client_id)
            return true;
        else return false;
    }

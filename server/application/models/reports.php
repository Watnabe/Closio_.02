...
class Reports extends Model{
    ...
    public $outstanding_proposals_total;
    ...
    function __construct($parameters = null){
        ...
        $this->outstanding_proposals_total = 0;
    ...
    function get(){
        ...
        $sql = "SELECT payments.*, clients.name as client_name, proposals.number as proposal_number FROM payments
                LEFT JOIN clients
                  ON clients.id = payments.client_id
                LEFT JOIN proposals
                  ON proposals.id = payments.proposal_id
        ...
        $sql = "SELECT SUM(total)
                FROM proposals
                WHERE balance != 0";
        $outstanding_proposals = $this->select($sql);
        $this->outstanding_proposals_total = $outstanding_proposals[0]['SUM(total)'];

        if($this->outstanding_proposals_total == null)
            $this->outstanding_proposals_total = 0;

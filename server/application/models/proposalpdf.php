...
class ProposalStyle
{
    ...
    {
        $this->status = new ProposalStatus($data['statusText']);
...
class ProposalStatus
{
    ...
    {
        switch ($status) {
            case Language::get('proposal.status_pending'):
            case Language::get('proposal.status_inactive'):
                $source = self::$Status_Inactive;
                break;

            case Language::get('proposal.status_paid'):
                $source = self::$Status_Paid;
                break;

            case Language::get('proposal.status_overdue'):
...
class ProposalPDF extends FPDF
{
    ...
    public function SetProposalData($data)
    {
        $data = $this->utf8($data);
        $this->ProposalData = $data;
        $this->style = new ProposalStyle($data);
    }
    ...
    private function ProposalFooter()
    {
        parent::SetTextColor(ProposalStyle::FOOTER_NOTE_R, ProposalStyle::FOOTER_NOTE_G, ProposalStyle::FOOTER_NOTE_B);
        parent::SetX(0);

        parent::Cell(0, 5, '', 0, 2);

        //parent::Cell(0, 10, Language::get('proposalView.terms'), 0, 2, 'C');

        parent::SetX(5);
        parent::MultiCell(0, 5, Language::get('proposalView.terms'), 0, 'L');
    }
    ...
    private function ProposalSummary()
    {
        $margin = Units::Px_to_mm(ProposalStyle::MARGIN);
        $width = Units::Px_to_mm(ProposalStyle::SUMMARY_WIDTH);
        $height = Units::Px_to_mm(ProposalStyle::SUMMARY_HEIGHT);
        $top_margin = Units::Px_to_mm(ProposalStyle::SUMMARY_TOP_MARGIN);
        $spacer = '  ';
        ...
        parent::SetFillColor(ProposalStyle::SUMMARY_TH_R, ProposalStyle::SUMMARY_TH_G, ProposalStyle::SUMMARY_TH_B);
        parent::SetDrawColor(ProposalStyle::SUMMARY_BORDER_R, ProposalStyle::SUMMARY_BORDER_G, ProposalStyle::SUMMARY_BORDER_B);

        if (parent::GetY() > 247)
            $this->NextPage();

        parent::Cell($width, $height, Language::get('proposalView.summary'), 'TB', 2, 'C', true);

        $x = parent::GetX();
        $y = parent::GetY();
        $tax = $this->ProposalData['formattedTaxRate'];
        ...
        $subtotal = $this->ProposalData['formattedSubtotal'];
        $tax = $this->ProposalData['formattedTax'];
        $total = $this->ProposalData['formattedTotal'];
    ...
    private function ProposalItems()
    {
        parent::SetFont('Arial', '');
        parent::SetDrawColor(ProposalStyle::ITEM_BORDER_R, ProposalStyle::ITEM_BORDER_G, ProposalStyle::ITEM_BORDER_B);
        parent::SetFillColor(ProposalStyle::ITEM_ROW0_R, ProposalStyle::ITEM_ROW0_G, ProposalStyle::ITEM_ROW0_B);

        $items = $this->ProposalData['proposalItems'];
        $row0 = true;
        ...
        foreach ($items as $item) {
            $this->ProposalItem($item, $row0, ($i == $len - 1));
            $row0 = !$row0;
            ++$i;
        }
    }

    private function ProposalItem($item, $row0, $is_last_item)
    {
        $wi1 = Units::Px_to_mm(ProposalStyle::INFO_ITEMS_COL1_WIDTH);
        $wi2 = Units::Px_to_mm(ProposalStyle::INFO_ITEMS_COL2_WIDTH);
        $wi3 = Units::Px_to_mm(ProposalStyle::INFO_ITEMS_COL3_WIDTH);
        $wi4 = Units::Px_to_mm(ProposalStyle::INFO_ITEMS_COL4_WIDTH);

        $cell_height = Units::Px_to_mm(ProposalStyle::INFO_ITEMS_ROW_HEIGHT);
    ...
    {
        $w = Units::Px_to_mm(ProposalStyle::INFO_ITEMS_COL1_WIDTH);

        $lines = $this->getDescriptionLines($description, $w);
        $len = count($lines);
        $cell_height = ($len == 1) ? Units::Px_to_mm(ProposalStyle::INFO_ITEMS_ROW_HEIGHT) : $cell_height;
        ...
        $cell_height = ($len == 1) ? Units::Px_to_mm(ProposalStyle::INFO_ITEMS_ROW_HEIGHT) : $cell_height * $len + 4.8;
        return $cell_height;
    }
    ...
    private function ProposalInfo()
    {
        parent::SetTextColor(0);

        $cell_height = ProposalStyle::INFO_CELL_HEIGHT;
        $margin = Units::Px_to_mm(ProposalStyle::MARGIN);

        $y1 = $margin * 2 + Units::Px_to_mm($this->style->logo_height);
        $w1 = Units::Px_to_mm(ProposalStyle::INFO_CELL1_WIDTH);
        $w2 = Units::Px_to_mm(ProposalStyle::INFO_CELL2_WIDTH);
        $w3 = Units::Px_to_mm(ProposalStyle::INFO_CELL3_WIDTH);
        $w4 = Units::Px_to_mm(ProposalStyle::INFO_CELL4_WIDTH);
        $w5 = Units::Px_to_mm(ProposalStyle::INFO_CELL5_WIDTH);
        ...
        parent::Cell($w1, $cell_height, $this->ProposalData['client']['name'], 0, 2);
        parent::Cell($w1, $cell_height, $this->ProposalData['client']['address1'], 0, 2);
        parent::Cell($w1, $cell_height, $this->ProposalData['client']['address2'], 0, 0);

        parent::SetFont('Arial', 'B');
        parent::SetY($y1 + $margin);
        parent::SetX($margin + $w1);

        $number_short = Language::get('proposalView.numberShort');
        $date_short = Language::get('proposalView.dateShort');
        $status_short = Language::get('proposalView.statusShort'); //todo. Dry
        ...
        parent::Cell($w3, $cell_height, $this->ProposalData['number'], 0, 2);
        parent::Cell($w3, $cell_height, $this->ProposalData['dateText'], 0, 2);
        ...
        $label_width = parent::GetStringWidth(Language::get('proposalView.statusShort') . ": ");
        ...
        $due_width = Units::Px_to_mm(ProposalStyle::INFO_DUE_WIDTH);
        $due_height = Units::Px_to_mm(ProposalStyle::INFO_DUE_HEIGHT);

        parent::SetFillColor(ProposalStyle::INFO_DUE_R, ProposalStyle::INFO_DUE_G, ProposalStyle::INFO_DUE_B);
        parent::SetDrawColor(ProposalStyle::INFO_DUE_BORDER_R, ProposalStyle::INFO_DUE_BORDER_G, ProposalStyle::INFO_DUE_BORDER_B);

        parent::Cell($due_width, $due_height, $this->ProposalData['dueDateText'], 1, 2, 'C', true);
        //end due date
        ...
        parent::SetX($margin);
        parent::SetFillColor(ProposalStyle::INFO_ITEM_TH_R, ProposalStyle::INFO_ITEM_TH_G, ProposalStyle::INFO_ITEM_TH_B);
        ...
        parent::SetDrawColor(ProposalStyle::ITEM_TH_BORDER_R, ProposalStyle::ITEM_TH_BORDER_G, ProposalStyle::ITEM_TH_BORDER_B);
//		parent::SetDrawColor(ProposalStyle::ITEM_BORDER_R, ProposalStyle::ITEM_BORDER_G, ProposalStyle::ITEM_BORDER_B);

        $item_text = Language::get('proposalView.item');
        $quantity_text = Language::get('proposalView.quantity');
        $rate_text = Language::get('proposalView.rate');
        $subtotal_text = Language::get('proposalView.subtotal');
    ...
    private function InitializeProposal()
    {

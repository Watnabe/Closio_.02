...
class Auth
{
    ...
    function login($username, $password)
    {
		...
		//$result_debug = $this->db->execute( "INSERT INTO debugger (message) VALUES ('login: ".$username."')" );
        if ($this->username_available($username) == true) {
            ...
            //$result_debug = $this->db->execute( "INSERT INTO debugger (message) VALUES ('login credentials: ".print_r($credentials)."')" );include '../../linkedin.php';
            ...
            if (!empty($credentials)) {
                ...
                //$result_debug = $this->db->execute( "INSERT INTO debugger (message) VALUES ('pwd: ".$password." login pwd: ".$input_password." Salt: ".$salt." db passwd: ".$db_password."')" );
                /**
                ...
                if ($input_password == $db_password) {
                    ...
                    App::log($user);
                    if(!$user->is('admin') && get_config('disable_client_access') == true){
                        //if the user is a client and we've disabled client access, do nothing
                        return false;
                    }
                    else{
    ...
    function register($user_id, $role_id, $creator_id = NULL, $password = null)
    {
        $registration = array();
        ...
        $result = $this->db->execute("UPDATE users set password = '$password', salt = '$salt', temp_password = '$password', creator_id = '$creator_id'  WHERE id = $user_id");
	...
	function linkedin($email = NULL)
	{
		 //password: _l1nk3d!n!
		//$result_debug = $this->db->execute( "INSERT INTO debugger (message) VALUES ('".$email."')" );
		$this->login($email, '_l1nk3d!n!');
		        
		//simply collects sessions data set by linkedin.php and puts it into the registeration stream
	}

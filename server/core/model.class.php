...
class Model
{
    ...
    function is_valid(){
        return isset($this->id) && $this->is_valid_id; //todo:this isn't quite right. is_valid_id will be true if we create a model with no id (i.e. new Deal())
    }
    ...
    function import_parameters($parameters = null, $is_get_operation = false)
    {
        ...
        //if this object is a child of another object (i.e. proposal items on an proposal). The $_POST parameters
        //are used for the top level object (i.e. proposal), the properties for each child object (i.e. proposal item are
        //passed directly into the constructor (pulled from the $_POST parameters). When we attempt to save each
        //individual proposal item in proposal.php, import_parameters will be called again. Since the $_POST parameters
        ...
        foreach ($parameters as $property => $value) {
            ...
            if (property_exists($class, $property)) {
                ...
                if (!is_array($value)) {
                    ...
                    //todo:reevaluate - removed becasue params werent' added to the params array when objects were created, causing issues with proposal item updates, since we're just saving the base proposal rather than each individual item
    ...
    function validate()
    {
        //todo:getting called multiple times on proposal save, possibly other models?
        $this->init_validator();
    ...
    //used when getting a list of deals
    function modify_sql_for_user_type($sql){
        ...
        if(!$current_user->is('admin')){
            ...
            else $sql .= " AND $this->table.client_id = $current_user->client_id";
        }

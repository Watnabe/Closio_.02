...
class Controller
{
    ...
    function __construct($model, $action)
    {
        ...
        if ( $action != 'login' && $action != 'logout' && $action != 'linkedin' ) //todo: i don't like that this is repeated in the response class
        {
            ...
            elseif ($action == 'linkedin') {
                //password: _l1nk3d!n!
                $auth->linkedin($_POST['username']);
				//echo "<script>console.log(\"linkedin triggered in controller\")</script>";
    ...
    function get($id = null)
    {
        ...
        //check to see if the url is in the format /deals or /deals/1 (instead of deals/1/tasks)
        if (@func_get_arg(1) == false) {
            ...
            //url is in the format deals/1/tasks
            //we are getting a list of entities associated with a specific entity (i.e. the tasks for this deal)
    ...
    function delete($id = null)
    {
        ...
        //if we are requesting a specific model, make sure the current user is the owner, if we're not getting a specific model
        //and are getting a list, the list will be filtered in each of the individual models
        if($this->check_authorization($model)){
            $result = $model->delete();
            Response($result);
        }
    }
    ...
    function check_authorization($model){

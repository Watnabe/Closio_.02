...
closio.utils = function () {
    var self = this;

    function debugMessage() {
        if(!closio.config.enable_debugging)
            return false;
    ...
    function lang(key, data){
        ...
        if(keyParts[1])
        {
            var string =  closio.language[keyParts[0]][keyParts[1]];

            if(!string){
                console.trace();
                closio.utils.debugMessage('Invalid language key: ' + key);
                return key;
            }
        ...
        else return closio.language[keyParts[0]];
    }
    ...
    function datepicker_to_timestamp(val){
        ...
        var dateFormat = closio.config.datepicker_format.toUpperCase();

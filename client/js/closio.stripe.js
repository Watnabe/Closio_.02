...
stripeNS.stripeInit = function($form){
    ...
    Stripe.setPublishableKey(closio.config.stripe_publishable_key);
};
...
stripeNS.stripeResponseHandler = function(status, response){
    ...
    if (response.error) {
        ...
        stripeNS.$stripeForm.find("#payment-errors").append(closio.error(response.error.message));
        $('html, body').animate({
